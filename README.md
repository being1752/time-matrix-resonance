...
event_snapshots.xlsx # 开平仓、区间变化与持仓逐分钟快照
summary.xlsx     # 回测总结
...
| 项目名 | 含义 | 主要用途 |
| --- | --- | --- |
| `summary.xlsx` | 回测总结 | 看整体有没有交易、胜率、盈亏、是否有未平仓 |
| `signals.xlsx` | 开平仓信号 | 看每一笔真实开仓和平仓的时间、价格、原因 |
| `intervals.xlsx` | 共振区间变化 | 看满足条件的区间什么时候出现、变化、消失 |
| `event_snapshots.xlsx` | 事件与持仓逐分钟快照 | 看开仓后每根 1 分钟线、持仓状态、关键周期状态 |
| `signal_validation.xlsx` | 算法信号后验验证 | 看共振信号出现后，价格是否按预期方向运行 |
| `chart_data.json` | 前端图表数据 | 给 K 线复盘页面使用，不建议手工分析 |
| `backtest.log` | 运行日志 | 排查配置、进度、区间变化、开仓、平仓和异常 |

`summary.xlsx`：

| 项目名 | 含义 |
| --- | --- |
| 分钟线数量 | 本次实际参与回测的 1 分钟 K 线数量 |
| 信号数量 | 开仓信号和平仓信号总数 |
| 区间变化数量 | 最优共振区间发生变化的次数 |
| 事件快照数量 | `event_snapshots.xlsx` 中记录的快照行数 |
| 算法验证数量 | 创建了多少次算法后验验证任务 |
| 算法验证成功数量 | 验证窗口结束时，价格符合预期方向的次数 |
| 算法验证成功率百分比 | 算法验证成功数量 / 算法验证数量 |
| 完整交易笔数 | 有开仓也有平仓的完整交易数量 |
| 盈利笔数 | 完整交易中，平仓价高于开仓价的笔数 |
| 亏损笔数 | 完整交易中，平仓价低于开仓价的笔数 |
| 胜率百分比 | 盈利笔数 / 完整交易笔数 |
| 总盈亏 | 所有完整交易的价差盈亏合计，未计算手续费和滑点 |
| 平均每笔盈亏 | 总盈亏 / 完整交易笔数 |
| 是否有未平仓 | 回测结束时是否还有持仓没有平掉 |
...
| 项目名 | 含义 |
| --- | --- |
| 时间 | 开仓或平仓信号发生的 1 分钟 K 线时间 |
| 动作 | `open_long` 表示开多，`close_long` 表示平多 |
| 价格 | 信号价格，当前使用当根 1 分钟 K 线收盘价 |
| 原因 | 信号触发原因 |
| 区间最小周期 | 信号发生时锁定区间的 `P_min` |
| 区间最大周期 | 信号发生时锁定区间的 `P_max` |
| 触发周期 | 负责触发开平仓的小周期 `P_trigger` |
| 触发前颜色 | 触发周期上一状态颜色 |
| 触发后颜色 | 触发周期当前状态颜色 |
...
| 时间 | 最优共振区间发生变化的时间 |
| 方向 | 区间方向，当前主要是 `long` |
| 区间最小周期 | 当前最优区间的 `P_min` |
| 区间最大周期 | 当前最优区间的 `P_max` |
| 区间长度 | `P_max - P_min + 1` |
| 趋势周期数 | 区间内趋势为 `up` 的周期数量 |
| 最大连续趋势数 | 区间内连续 `up` 周期的最大长度 |
| 趋势占比 | 趋势周期数 / 区间总周期数，分母包含 `up/down/non` |
| 触发周期 | 根据 `P_min` 和配置除数算出的 `P_trigger` |

`event_snapshots.xlsx`：

| 项目名 | 含义 |
| --- | --- |
| 事件时间 | 当前快照对应的 1 分钟 K 线时间 |
| 事件类型 | `interval_change`、`open_long`、`position_tick`、`close_long` |
| 原因 | 本行快照产生的原因 |
| 价格 | 当前事件价格，通常等于当根 1 分钟收盘价 |
| 1分钟开盘价 | 当前 1 分钟 K 线开盘价 |
| 1分钟最高价 | 当前 1 分钟 K 线最高价 |
| 1分钟最低价 | 当前 1 分钟 K 线最低价 |
| 1分钟收盘价 | 当前 1 分钟 K 线收盘价 |
| 1分钟成交量 | 当前 1 分钟 K 线成交量 |
| 交易日 | 当前 K 线所属交易日 |
| 当日有效分钟序号 | 当前 K 线在交易日内的有效分钟累计序号，午休不计入 |
| 持仓状态 | `flat` 表示空仓，`long` 表示持有多单 |
| 持仓开仓时间 | 当前持仓从哪根 1 分钟 K 线开始 |
| 持仓开仓价 | 当前持仓的开仓价 |
| 持仓浮盈亏 | 当前价格 - 开仓价，未计算手续费和滑点 |
| 持仓最高价 | 开仓后到当前快照为止出现过的最高价 |
| 持仓最低价 | 开仓后到当前快照为止出现过的最低价 |
| 旧区间最小周期 | 区间变化前的 `P_min` |
| 旧区间最大周期 | 区间变化前的 `P_max` |
| 旧区间长度 | 区间变化前的区间长度 |
| 旧趋势周期数 | 区间变化前，趋势为 `up` 的周期数量 |
| 旧趋势占比 | 区间变化前的趋势占比 |
| 新区间最小周期 | 区间变化后或当前持仓锁定的 `P_min` |
| 新区间最大周期 | 区间变化后或当前持仓锁定的 `P_max` |
| 新区间长度 | 区间变化后或当前持仓锁定的区间长度 |
| 新趋势周期数 | 新区间内趋势为 `up` 的周期数量 |
| 新最大连续趋势数 | 新区间内连续 `up` 周期的最大长度 |
| 新趋势占比 | 新区间趋势周期数 / 新区间总周期数 |
| 触发周期 | 负责开平仓的小周期 `P_trigger` |
| 触发周期MACD | `P_trigger` 当前最新 MACD 柱值 |
| 触发前颜色 | 触发周期上一状态颜色 |
| 触发后颜色 | 触发周期当前状态颜色 |
| 触发周期趋势 | `P_trigger` 当前趋势状态 |
| 最小周期MACD | `P_min` 当前最新 MACD 柱值 |
| 最小周期颜色 | `P_min` 当前颜色状态 |
| 最小周期趋势 | `P_min` 当前趋势状态 |
| 最大周期MACD | `P_max` 当前最新 MACD 柱值 |
| 最大周期颜色 | `P_max` 当前颜色状态 |
| 最大周期趋势 | `P_max` 当前趋势状态 |
...
| 方向 | 验证方向，当前主要是 `long` |
| 区间最小周期 | 本次验证锁定的 `P_min` |
| 区间最大周期 | 本次验证锁定的 `P_max` |
| 区间长度 | 验证起点的区间长度 |
| 触发周期 | 本次区间对应的 `P_trigger` |
| 最小周期已运行分钟 | 当前 `P_min` 周期已经运行了多少有效分钟 |
| 预期观察分钟 | 按 `P_max - P_min - 已运行分钟` 算出的观察窗口 |
| 实际观察分钟 | 实际完成观察的有效分钟数 |
| 起始价格 | 验证起点价格 |
| 结束时间 | 验证窗口结束时间 |
| 结束价格 | 验证窗口结束价格 |
| 结束涨跌幅百分比 | 结束价格相对起始价格的涨跌幅 |
| 窗口最高价 | 观察窗口内最高价 |
| 最高价出现时间 | 窗口最高价出现的时间 |
| 最高价出现于第几根有效分钟 | 最高价出现在观察窗口第几根有效 1 分钟线 |
| 最大浮盈百分比 | 窗口最高价相对起始价格的最大浮盈 |
| 窗口最低价 | 观察窗口内最低价 |
| 最低价出现时间 | 窗口最低价出现的时间 |
| 最低价出现于第几根有效分钟 | 最低价出现在观察窗口第几根有效 1 分钟线 |
| 最大回撤百分比 | 窗口最低价相对起始价格的最大回撤 |
| 是否符合预期方向 | `long` 方向下，结束价格是否高于起始价格 |
| 起始趋势周期数 | 验证开始时，区间内 `up` 周期数量 |
| 结束趋势周期数 | 验证结束时，区间内 `up` 周期数量 |
| 窗口内最小趋势周期数 | 观察窗口内出现过的最小 `up` 周期数量 |
| 窗口内最大趋势周期数 | 观察窗口内出现过的最大 `up` 周期数量 |
| 起始趋势占比 | 验证开始时的趋势占比 |
| 结束趋势占比 | 验证结束时的趋势占比 |
| 趋势周期数变化 | 结束趋势周期数 - 起始趋势周期数 |
| 趋势强弱变化 | `stronger`、`weaker`、`unchanged` |
| 结束时区间是否有效 | 观察结束时原始区间是否仍满足离场配置 |
| 是否完整观察 | 是否走满预期观察窗口 |
| 完成原因 | `completed_window` 表示走满窗口，`end_of_data` 表示数据结束 |
...
`event_snapshots.xlsx` 用来复盘关键变化点，当前会记录四类事件：
